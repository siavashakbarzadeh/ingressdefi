...
router.post('/createWithdraw', common.originMiddleware, async function (req, res){
    try{
     
        let reqData = req.body;
        
        let transactionExists = await withdraw.findOne({txId:reqData.txId}).lean();
      
        if(!transactionExists){
            let holdersDetails = await withdraw.create(reqData);
...
router.post('/updateWithdraw', common.originMiddleware, async function(req, res){
    try{
        let reqData = req.body;
        let updateDetails = await withdraw.update({_id:reqData._id},{$set:{status:reqData.status}});
        if(updateDetails){
...
router.post('/withdrawlist', async function(req,res){
      ...
      if(req.body.search != ""){
       ...
       async.parallel({
        holdersTotalCount:function(cb) {
            withdraw.find(cnt).countDocuments().exec(cb);
        },
        holdersData:function(cb) {
            withdraw.find( cnt ).limit(limit).skip(skip).sort( srt ).lean().exec(cb)
  ...
  cron.schedule('* * * * *', async () => {
    let withdrawData = await withdraw.find({status:'Pending'});
    if(withdrawData && withdrawData.length > 0){
      for(let i=0; i<withdrawData.length; i++){
        let txid        = withdrawData[i].txId;
        web3.eth.getTransactionReceipt(txid, async function (err, result) {
          if(!err) {
            if(result.status == true) {
                let updateWithdraw = await withdraw.update({txId:txid},{$set:{status:'Success'}});
            } else {
                let updateFailwithdraw = await withdraw.update({txId:txid},{$set:{stauts:'Failed'}});
            }
          } else {
